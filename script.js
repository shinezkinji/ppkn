...
// ================= Reveal saat scroll (berulang: keluar-masuk setiap kali dilewati) =================
// rootMargin atas dibuat negatif seukuran tinggi navbar fixed, supaya elemen baru
// dianggap "terlihat" saat benar-benar muncul di bawah navbar — bukan saat masih
// tertutup navbar. Ini yang bikin animasinya konsisten terulang tiap kali discroll
// turun-naik-turun lagi, bukan cuma sekali muncul di awal.
...
function buildRevealObserver() {
  ...
  return new IntersectionObserver(
    ...
    { threshold: 0.12, rootMargin: `-${navH + 20}px 0px -12% 0px` }
  );
}
let revealObserver = buildRevealObserver();
revealEls.forEach((el) => revealObserver.observe(el));

// Navbar bisa berubah tinggi (mode "scrolled"), rebuild observer saat resize
// supaya titik trigger tetap akurat.
let resizeTimer;
...
if (footerStempel) {
  const stempelObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        entry.target.classList.toggle('stamp-in', entry.isIntersecting);
      });
    },
    { threshold: 0.4 }
  );
